...
def plot_numPosition(df):
    ...
    positionCounts=AgenPositions.groupby(AgenPositions.Agency).sum()#add up the total number of open position by agency
    sorted_Counts=positionCounts.sort_index(by=['# Of Positions'],ascending=False)
    sorted_Counts.head(10).plot(kind='barh' ,fontsize=12,title='Top 10 Agencies hiring the most \n (Date between %s and %s) '%(postTime.min(),postTime.max()))#plot top 10 agency
    plt.show()
